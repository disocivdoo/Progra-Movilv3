...
export class AuthService {
  ...
  storageClassDataKey = 'CLASS_DATA';  // Nueva clave para datos de clase
  ...
  // Método para limpiar los datos de clase
  async clearClassData(): Promise<void> {
    try {
      await this.storage.remove(this.storageClassDataKey);
      sessionStorage.removeItem('asistencia');
    } catch (error) {
      showAlertError('AuthService.clearClassData', error);
    }
  }
  ...
  async logout(): Promise<boolean> {
    try {
      ...
      if (user) {
        ...
        await this.clearClassData(); // Limpiar datos de clase al cerrar sesión
      }
  ...
  getCurrentUsername(): string | null {
    return this.authUser.value?.userName ?? null;
  }
}